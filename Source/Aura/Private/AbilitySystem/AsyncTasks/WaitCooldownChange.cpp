...
UWaitCooldownChange* UWaitCooldownChange::WaitForCooldownChange(UAbilitySystemComponent* AbilitySystemComponent, const FGameplayTag& InCooldownTag)
{
	...
	//We bind a function so we can know when the cooldown has changed
	...
	//Bind the delegate when a cooldown effect has been applied
...
void UWaitCooldownChange::CooldownTagChanged(const FGameplayTag InCooldownTag, int32 NewCount)
{
	...
	{
		//We are out of time
		CooldownSignatureEnd.Broadcast(0.f);
	}
}
...
void UWaitCooldownChange::OnActiveEffectAdded(UAbilitySystemComponent* TargetASC, const FGameplayEffectSpec& SpecApplied, FActiveGameplayEffectHandle ActiveEffectHandle)
{
	...
	if (AssetTags.HasTagExact(CooldownTag) || GrantedTags.HasTagExact(CooldownTag))
	{
		...
		if (TimesRemaining.Num()>0)
		{
			...
			CooldownSignatureStart.Broadcast(TimeRemaining);
		}
	}
}
