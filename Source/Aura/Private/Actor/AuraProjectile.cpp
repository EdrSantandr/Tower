...
{
	if (!IsValid(DamageEffectParams.SourceAbilitySystemComponent) || !DamageEffectParams.SourceAbilitySystemComponent->GetAvatarActor()) return;
	...
	if (HasAuthority())
	{
		...
		{
			// Block for death impulse
			const FVector DeathImpulse = GetActorForwardVector() * DamageEffectParams.DeathImpulseMagnitude;
			DamageEffectParams.DeathImpulse = DeathImpulse;
			// Block for Knockback force
			const bool bKnockback = FMath::RandRange(1,100) < DamageEffectParams.KnockbackChance;
			if (bKnockback)
			{
				FRotator Rotation = GetActorRotation();
				Rotation.Pitch = DamageEffectParams.KnockbackPitchAngle;
